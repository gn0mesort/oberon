...
namespace oberon::internal::linux::x11 {
  ...
  window_impl::window_impl(graphics_device& device, const std::string& title, const rect_2d& bounds) :
  m_parent{ &static_cast<graphics_device_impl&>(device.implementation()) } {
    const auto connection = m_parent->wsi().connection();
    const auto screen = m_parent->wsi().default_screen();
    ...
    // Configure XInput 2 events.
    {
      ...
      masks[0].head.deviceid = m_parent->wsi().keyboard();
      ...
      masks[1].head.deviceid = m_parent->wsi().pointer();
    ...
    OBERON_CHECK_ERROR_MSG(!nng_sub0_open(&m_socket), 1, "Failed to open an NNG subscriber socket.");
    {
      ...
      auto leader = m_parent->wsi().leader();
    ...
    {
      const auto instance = m_parent->wsi().instance_name();
      const auto application = m_parent->wsi().application_name();
      // Set WM_CLASS
      ...
      xcb_change_property(connection, XCB_PROP_MODE_REPLACE, m_window, m_parent->wsi().atom_by_name(WM_CLASS_ATOM),
                          XCB_ATOM_STRING, 8, instance.size() + 1, instance.c_str());
      xcb_change_property(connection, XCB_PROP_MODE_APPEND, m_window, m_parent->wsi().atom_by_name(WM_CLASS_ATOM),
                          XCB_ATOM_STRING, 8, application.size() + 1, application.c_str());
    }
    {
      const auto pid = getpid();
      xcb_change_property(connection, XCB_PROP_MODE_REPLACE, m_window, m_parent->wsi().atom_by_name(NET_WM_PID_ATOM),
      ...
      xcb_change_property(connection, XCB_PROP_MODE_REPLACE, m_window,
                          m_parent->wsi().atom_by_name(WM_CLIENT_MACHINE_ATOM), XCB_ATOM_STRING, 8,
                          std::strlen(buffer.nodename), &buffer.nodename[0]);
    }
    {
      const auto protocols_atom = m_parent->wsi().atom_by_name(WM_PROTOCOLS_ATOM);
      const auto atoms = std::array<xcb_atom_t, 2>{ m_parent->wsi().atom_by_name(WM_DELETE_WINDOW_ATOM),
                                                    m_parent->wsi().atom_by_name(NET_WM_PING_ATOM) };
    ...
    {
      const auto leader = m_parent->wsi().leader();
      xcb_change_property(connection, XCB_PROP_MODE_REPLACE, m_window,
                          m_parent->wsi().atom_by_name(WM_CLIENT_LEADER_ATOM), XCB_ATOM_WINDOW, 32, 1, &leader);
    }
    // Set WM_HINTS
    {
      ...
      hints.window_group = m_parent->wsi().leader();
      xcb_change_property(connection, XCB_PROP_MODE_REPLACE, m_window, m_parent->wsi().atom_by_name(WM_HINTS_ATOM),
    ...
    initialize_keyboard();
    const auto& dl = m_parent->dispatch_loader();
    const auto physical = m_parent->physical_device().handle();
    // Create Vulkan window surface.
    {
      ...
      VK_SUCCEEDS(vkCreateXcbSurfaceKHR(m_parent->instance_handle(), &surface_info, nullptr, &m_surface));
      VK_DECLARE_PFN(dl, vkGetPhysicalDeviceSurfaceSupportKHR);
      auto supported = VkBool32{ };
      VK_SUCCEEDS(vkGetPhysicalDeviceSurfaceSupportKHR(physical, m_parent->queue_family(), m_surface, &supported));
      OBERON_CHECK_ERROR_MSG(supported, 1, "The selected device queue family (%u) does not support the "
                             "window.", m_parent->queue_family());
    }
    ...
    // Find available present modes.
    {
      ...
      VK_SUCCEEDS(vkGetPhysicalDeviceSurfacePresentModesKHR(m_parent->physical_device().handle(), m_surface, &sz,
                                                            nullptr));
  ...
  window_impl::~window_impl() noexcept {
    hide();
    const auto& dl = m_parent->dispatch_loader();
    const auto device = m_parent->device_handle();
    ...
    vkDestroySurfaceKHR(m_parent->instance_handle(), m_surface, nullptr);
    deinitialize_keyboard();
    xcb_destroy_window(m_parent->wsi().connection(), m_window);
    nng_dialer_close(m_dialer);
    nng_close(m_socket);
  }

  void window_impl::change_size_hints(const xcb_size_hints_t& hints) {
    const auto connection = m_parent->wsi().connection();
    xcb_change_property(connection, XCB_PROP_MODE_REPLACE, m_window, m_parent->wsi().atom_by_name(WM_NORMAL_HINTS_ATOM),
                        XCB_ATOM_WM_SIZE_HINTS, 32, sizeof(xcb_size_hints_t) >> 2, &hints);
  }


  void window_impl::initialize_swapchain(const VkSwapchainKHR old) {
    const auto& dl = m_parent->dispatch_loader();
    const auto device = m_parent->device_handle();
    VK_DECLARE_PFN(dl, vkDestroyImageView);
    for (auto& image_view : m_swapchain_image_views)
    ...
    VK_DECLARE_PFN(m_parent->dispatch_loader(), vkGetPhysicalDeviceSurfaceCapabilitiesKHR);
    VK_SUCCEEDS(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_parent->physical_device().handle(), m_surface,
                                                          &capabilities));
    ...
    auto queue_family = m_parent->queue_family();
  ...
  void window_impl::initialize_keyboard() {
    m_keyboard_map = xkb_x11_keymap_new_from_device(m_parent->wsi().keyboard_context(), m_parent->wsi().connection(),
                                                    m_parent->wsi().keyboard(), XKB_KEYMAP_COMPILE_NO_FLAGS);
    m_keyboard_state = xkb_x11_state_new_from_device(m_keyboard_map, m_parent->wsi().connection(),
                                                     m_parent->wsi().keyboard());
...
#undef OBERON_INTERNAL_LINUX_X11_MODIFIER_KEY_MAPPING
  ...
  void window_impl::deinitialize_keyboard() {
  ...
  u32 window_impl::id() const {
    return m_window;
  }

  void window_impl::show() {
    xcb_map_window(m_parent->wsi().connection(), m_window);
    xcb_flush(m_parent->wsi().connection());
  }

  void window_impl::hide() {
    xcb_unmap_window(m_parent->wsi().connection(), m_window);
    xcb_flush(m_parent->wsi().connection());
  }

  bitmask window_impl::query_visibility() const {
    const auto connection = m_parent->wsi().connection();
    const auto window_attrs_req = XCB_SEND_REQUEST(xcb_get_window_attributes, connection, m_window);
    const auto wm_state = m_parent->wsi().atom_by_name(WM_STATE_ATOM);
    ...
    const auto net_wm_state = m_parent->wsi().atom_by_name(NET_WM_STATE_ATOM);
    ...
    {
      const auto net_wm_state_hidden = m_parent->wsi().atom_by_name(NET_WM_STATE_HIDDEN_ATOM);
  ...
  bool window_impl::is_shown() const {
  ...
  bool window_impl::is_minimized() const {
    const auto bits = query_visibility();
    return bits == (query_visibility_flag_bits::iconic_bit | query_visibility_flag_bits::hidden_bit);
  }

  void window_impl::change_ewmh_states(const ewmh_state_action action, const xcb_atom_t first,
    ...
    client_message.type = m_parent->wsi().atom_by_name(NET_WM_STATE_ATOM);
    ...
    send_client_message(m_parent->wsi().default_screen()->root, generic);
  }

  void window_impl::change_compositor_mode(const compositor_mode mode) {
      xcb_change_property(m_parent->wsi().connection(), XCB_PROP_MODE_REPLACE, m_window,
                          m_parent->wsi().atom_by_name(NET_WM_BYPASS_COMPOSITOR_ATOM), XCB_ATOM_CARDINAL, 32, 1, &mode);
  }
  ...
  void window_impl::display_style(const enum display_style style) {
    ...
    const auto above = m_parent->wsi().atom_by_name(NET_WM_STATE_ABOVE_ATOM);
    const auto fullscreen = m_parent->wsi().atom_by_name(NET_WM_STATE_FULLSCREEN_ATOM);
    switch (style)
    {
  ...
  enum display_style window_impl::display_style() const {
    ...
    auto net_wm_bypass_req = XCB_SEND_REQUEST(xcb_get_property, connection, false, m_window,
                                              m_parent->wsi().atom_by_name(NET_WM_BYPASS_COMPOSITOR_ATOM),
    ...
    auto flags = bitmask{ };
    const auto above_atom = m_parent->wsi().atom_by_name(NET_WM_STATE_ABOVE_ATOM);
    const auto fullscreen_atom = m_parent->wsi().atom_by_name(NET_WM_STATE_FULLSCREEN_ATOM);
  ...
  rect_2d window_impl::drawable_rect() const {
    ...
    const auto root = m_parent->wsi().default_screen()->root;
  ...
  void window_impl::send_client_message(const xcb_window_t destination, const xcb_generic_event_t& message) {
  ...
  void window_impl::title(const std::string& title) {
    ...
    xcb_change_property(connection, XCB_PROP_MODE_REPLACE, m_window, m_parent->wsi().atom_by_name(NET_WM_NAME_ATOM),
                        m_parent->wsi().atom_by_name(UTF8_STRING_ATOM), 8, title.size(), title.c_str());
    xcb_flush(connection);
  }

  std::string window_impl::title() const {
    const auto connection = m_parent->wsi().connection();
    const auto utf = m_parent->wsi().atom_by_name(UTF8_STRING_ATOM);
    auto reply = ptr<xcb_get_property_reply_t>{ };
    XCB_SEND_REQUEST_SYNC(reply, xcb_get_property, connection, false, m_window,
                          m_parent->wsi().atom_by_name(NET_WM_NAME_ATOM), utf, 0, 1024 << 2);
  ...
  void window_impl::resize(const extent_2d& extent) {
    const auto connection = m_parent->wsi().connection();
    auto hints = xcb_size_hints_t{ };
  ...
  void window_impl::move_to(const offset_2d& offset) {
    const auto connection = m_parent->wsi().connection();
    ...
    case oberon::event_type::platform:
      {
        ...
        if (m_parent->wsi().is_keyboard_event(type))
        {
        ...
        case XCB_CLIENT_MESSAGE:
          {
            ...
            if (client_message->type == m_parent->wsi().atom_by_name(WM_PROTOCOLS_ATOM) && client_message->format == 32 &&
                client_message->data.data32[0] == m_parent->wsi().atom_by_name(NET_WM_PING_ATOM))
            {
  ...
  oberon::key window_impl::translate_keycode(const u32 code) const {
    return code >= m_to_external_key.size() ? oberon::key::none : m_to_external_key[code];
  }

  oberon::mouse_button window_impl::translate_mouse_buttoncode(const u32 code) const {
  ...
  bool window_impl::is_key_pressed(const oberon::key k) const {
  ...
  bool window_impl::is_key_echoing(const oberon::key k) const {
    if (k == oberon::key::none)
    {
      return false;
    }
  ...
  bool window_impl::is_mouse_button_pressed(const oberon::mouse_button mb) const {
  ...
  const std::unordered_set<presentation_mode>& window_impl::available_presentation_modes() const {
  ...
  u32 window_impl::acquire_next_image(const VkSemaphore acquired) {
    ...
    VK_DECLARE_PFN(m_parent->dispatch_loader(), vkAcquireNextImageKHR);
    auto res = u32{ };
    const auto status = vkAcquireNextImageKHR(m_parent->device_handle(), m_swapchain, VK_FOREVER, acquired,
                                              VK_NULL_HANDLE, &res);
    switch (status)
    {
  ...
  const std::vector<VkImage>& window_impl::swapchain_images() {
    return m_swapchain_images;
  }

  VkFormat window_impl::surface_format() const {
    return m_swapchain_surface_format.format;
  }

  VkExtent2D window_impl::swapchain_extent() const {
    return m_swapchain_extent;
  }

  void window_impl::present_image(const u32 index, const VkSemaphore ready_to_present) {
    ...
    if (ready_to_present)
    {
      ...
      present_info.pWaitSemaphores = &ready_to_present;
    }
    present_info.pImageIndices = &index;
    VK_DECLARE_PFN(m_parent->dispatch_loader(), vkQueuePresentKHR);
    auto status = vkQueuePresentKHR(m_parent->queue(), &present_info);
    switch (status)
    {
    case VK_ERROR_OUT_OF_DATE_KHR:
    case VK_SUBOPTIMAL_KHR:
