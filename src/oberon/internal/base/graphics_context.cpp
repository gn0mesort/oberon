...
#define VK_DECLARE_PFN(dl, pfn) OBERON_INTERNAL_BASE_VK_DECLARE_PFN(dl, pfn)
...
namespace oberon::internal::base {
  ...
  graphics_context::graphics_context(const std::string& application_name, const u32 application_version,
                                     const std::string& engine_name, const u32 engine_version,
                                     const std::unordered_set<std::string>& requested_layers,
                                     const std::unordered_set<std::string>& required_extensions,
                                     const std::unordered_set<std::string>& requested_extensions) {
    check_instance_version(m_dl);
    auto app_info = pack_application_info(application_name.c_str(), application_version, engine_name.c_str(),
                                          engine_version);
  ...
  const std::vector<VkPhysicalDevice>& graphics_context::physical_devices() {
    if (m_physical_devices.empty())
    {
      auto sz = u32{ };
      VK_DECLARE_PFN(m_dl, vkEnumeratePhysicalDevices);
      VK_SUCCEEDS(vkEnumeratePhysicalDevices(m_dl.loaded_instance(), &sz, nullptr));
      m_physical_devices.resize(sz);
      VK_SUCCEEDS(vkEnumeratePhysicalDevices(m_dl.loaded_instance(), &sz, m_physical_devices.data()));
    }
    return m_physical_devices;
  }

  debug_graphics_context::debug_graphics_context(const std::string& application_name, const u32 application_version,
                                                 const std::string& engine_name, const u32 engine_version,
                                                 const std::unordered_set<std::string>& requested_layers,
                                                 const std::unordered_set<std::string>& required_extensions,
                                                 const std::unordered_set<std::string>& requested_extensions) :
