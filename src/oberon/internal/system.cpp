...
namespace oberon::internal {
  ...
  void system_dtor::operator()(ptr<system> p) const noexcept {
    delete p;
  }

  system::system(const defer_construction&) { }

  system::system(const std::unordered_set<std::string>& requested_layers) {
#ifndef NDEBUG
    m_graphics_context.reset(new debug_graphics_context{ requested_layers, { }, { } });
#else
    m_graphics_context.reset(new graphics_context{ requested_layers, { }, { } });
#endif
    {
  ...
  system::~system() noexcept {
    m_graphics_device_ptrs.clear();
  }

  const std::vector<oberon::graphics_device>& system::graphics_devices() {
