...
namespace oberon {
  ...
  int application::run(const std::function<entry_point>& fn, const i32 argc, const ptr<csequence> argv) {
    ...
    try
    {
      OBERON_CHECK_ERROR_MSG(argv[argc] == nullptr, 1, "The application argument vector must be terminated with a "
                             "null value.");
...
// Linux, additionally, requires a window system to be selected for a complete system definition to be built.
      ...
      const auto engine_version = VK_MAKE_API_VERSION(0, configuration::version_major(),
                                                      configuration::version_minor(), configuration::version_patch());
  ...
  #ifndef NDEBUG
      auto vk_ctx = new internal::base::debug_graphics_context{ x11_ctx->instance_name(), 0,
                                                                x11_ctx->application_name(), engine_version,
                                                                requested_layers, required_extensions, { } };
  #else
  ...
  #error There must be at least one valid implementation available to the application.
      ...
      auto sys = system{ std::move(sys_impl) };
